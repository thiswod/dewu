...
- **自定义内容选项**：可选择只保留第一张图片，节省存储空间
- **自定义保存位置**：用户可选择保存目录
- **实时进度显示**：下载过程中显示进度和统计信息
- **完成通知**：下载完成后显示详细的成功/失败统计
- **线程安全**：使用原子操作确保多线程环境下的正确执行
...
### 下部区域
- **选项设置**：
  - 只保留第一张图片：勾选后只保存第一个视频/图片，节省存储空间
- **保存位置设置**：
  - 文本框：显示当前选择的保存目录路径
...
### 界面预览
![应用程序界面截图](img/Snipaste_2025-10-27_17-25-51.png)
...
- **线程安全处理**：使用Interlocked.CompareExchange实现原子操作，确保多线程环境下的正确执行
...
3. 请确保有足够的磁盘空间和写入权限
4. 下载速度取决于网络状况和服务器响应
5. 勾选"只保留第一张图片"选项时，只有第一个线程会保存图片，确保只下载一个副本
